...
/**
 * Invites a collaborator to a vimpl board by email.
 * Uses the existing POST /api/v1/boards/:id/share endpoint.
 *
 * @param {string} boardId
 * @param {string} email
 * @param {{ baseUrl: string, token: string }} config
 */
export async function inviteCollaborator(boardId, email, config) {
  const { baseUrl, token } = config;
  const res = await fetch(`${baseUrl}/api/v1/boards/${boardId}/share`, {
    method: 'POST',
    headers: {
...
export async function updateVimplBoard(boardId, projectPlan, processName, config, selectedImprovements = [], processDescription = null) {
  const { baseUrl, token } = config;

  const res = await fetch(`${baseUrl}/api/v1/boards/${boardId}/import`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
