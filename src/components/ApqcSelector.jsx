...
export default function ApqcSelector({ processContext, onChange, nodes: customNodes }) {
  ...
  function select(node) {
    onChange({ ...processContext, apqcNodeId: node.id, apqcNodeName: node.name, apqcNodeLevel: node.level, isCustom: false, customLabel: null });
    setOpen(false);
    setSearch('');
  }
              ...
              <div className="p-2 space-y-0.5">
                {matchedL2.length === 0 && matchedL3.length === 0 ? (
                  ...
                  <>
                    ...
                    {matchedL2.length > 0 && matchedL3.length === 0 && matchedL2.map(node => (
                      ...
                      {isExpL1 && (
                        <div className="mt-1.5 ml-2 space-y-0.5">
                          {l2s.map(l2 => {
                            ...
                            const hasL3 = l3s.length > 0;
                            return (
                              <div key={l2.id}>
                                {/* L2 — expands to L3 if children exist, selects only if leaf */}
                                <button
                                  onClick={() => hasL3 ? setExpandedL2(isExpL2 ? null : l2.id) : select(l2)}
                                  className={`w-full text-left text-[10px] px-2 py-1 rounded border transition-colors flex items-center gap-1 ${
                                    isSelL2
                                ...
                                >
                                  <span className="font-mono text-gray-400 mr-0.5">{l2.id}</span>
                                  <span className="flex-1">{l2.name}</span>
                                  {hasL3 && (
                                    <span className="text-gray-400 shrink-0">{isExpL2 ? '▴' : '▾'}</span>
                                  )}
                                </button>
                                {isExpL2 && (
